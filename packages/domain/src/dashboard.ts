...
export function buildDashboardSummary(records: DocumentRecord[], referenceDate: Date | string = new Date()): DashboardSummary {
  ...
  return {
    ...
    expirationBuckets: groupExpirationBuckets(processed, toDate(referenceDate))
  };
}
...
function getExpirationBucket(expirationDate: string | null, referenceDate: Date): string {
  if (!expirationDate) {
